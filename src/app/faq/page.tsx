...
const faqCategories = [
  {
    ...
    items: [
      ...
      { q: 'Can I track my order?', a: 'Yes! Once your order ships, you will receive an email with tracking information. You can also track it in your account under "My Orders".' },
    ],
  },
  {
    title: 'Products & Quality', icon: '✨',
    items: [
      { q: 'Are your products body-safe?', a: 'Yes. All ZENEIO products are made from medical-grade silicone, ABS plastic, or other body-safe materials. They are free of phthalates and latex.' },
  ...
  return (
    <div className="min-h-screen bg-zeneio-black">
      ...
      <div className="page-header">
        ...
        <p>Everything you need to know about ZENEIO</p>
      </div>
      ...
      <div className="section-container pb-20">
            ...
            <div key={cat.title} className="glass rounded-2xl overflow-hidden">
              <button
                ...
                className={`w-full flex items-center justify-between p-5 text-left transition-colors ${
                  openCategory === cat.title ? 'bg-white/[0.03]' : ''
                }`}
              >
                <span className="flex items-center gap-3 font-bold text-sm sm:text-base">
                  ...
                  {cat.items.map((item) => {
                    ...
                    return (
                      <div key={key} className="border-t border-white/5 pt-3 first:pt-0 first:border-t-0">
                        <button
                          onClick={() =>
                            setOpenQuestion((prev) => ({
                              ...prev,
                              [key]: isOpen ? null : item.q,
                            }))
                          }
                          className="w-full flex items-start gap-3 text-left py-2 group"
                        >
                          <span
                            className={`mt-1.5 w-4 h-4 rounded-full border flex-shrink-0 flex items-center justify-center transition-all ${
                              isOpen ? 'bg-zeneio-accent border-zeneio-accent' : 'border-white/20'
                            }`}
                          >
                            {isOpen && <span className="w-1.5 h-1.5 rounded-full bg-black" />}
                          </span>
                          <span
                        ...
                        {isOpen && (
                          <div className="pl-7 pr-2 pb-2 animate-fade-in">
        ...
        <div className="text-center mt-12 glass rounded-2xl p-8 max-w-md mx-auto">
          <HelpCircle size={32} className="mx-auto text-zeneio-accent/40 mb-3" />
          <p className="text-sm text-white/40 mb-4">Still have questions?</p>
          <Link href="/contact" className="btn-accent">Contact Our Team</Link>
        </div>
      </div>
